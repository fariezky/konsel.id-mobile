...
class _LocationErrorScreenState extends State<LocationErrorScreen> {
  ...
  Future locateUser() async {
    ...
    try {
      ...
      setState(() {
        _currentAddress =
            "${first.locality}, ${first.name},${first.postalCode}, ${first.street}";
      });
      //check delivery
      ...
      sharedPreferences.setString('FeatureName', first.name ?? '');
      var featureName = sharedPreferences.getString('FeatureName');
      sharedPreferences.setString('AddressLine', first.street ?? '');
